'''
Defines the app_customer used in chatbot_customer.
...
sqlbot: Answer questions related to only using the transactions of that particular user; access restriction for other user accounts
...
from agents.multiagent.customer.sql_agent_customer import sql_graph_customer
from agents.multiagent.customer.rag_agent_customer import rag_chain
...
def ragbot(state: AgentState):
    ...
    # rag with RetrievalQA.from_chain_type()
...
def sqlbot(state: AgentState):
      ...
      question = state["question"]
      sqlbot_response = sql_graph_customer.invoke({"sql_question": question,
...
app_customer = graph.compile(checkpointer=checkpointer)
